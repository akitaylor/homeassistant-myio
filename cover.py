    ...
    if _server_status.startswith("Online"):
        for _id in _prots:
            if 0 < _prots[_id]["element0"] <= 64 and 0 < _prots[_id]["element1"] <= 64:
                _cover_entities.append(
                    MyIOCover(hass, config_entry, _id, _server_name))
    async_add_entities(_cover_entities, True)


class MyIOCover(CoverEntity):
    ...
    def __init__(self, hass, config_entry, _id, _server_name):
        ...
        self._relay_up = str(
            self._server_data["protection"][str(_id)]["element0"])
        self._relay_down = str(
            self._server_data["protection"][str(_id)]["element1"])
        self._relay_up_d = str(
            self._server_data["relays"][str(
                int(self._relay_up) - 1)]["description"]
        )
        self._relay_down_d = str(
            self._server_data["relays"][str(
                int(self._relay_down) - 1)]["description"]
        )
        self._id = _id
        ...
        self._name = longest_substring_finder(
            self._relay_up_d, self._relay_down_d,)
        self.hass = hass
        self._unique_id = _id
